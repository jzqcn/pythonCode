...
class DownloadManager:
    def __init__(self, header, url, allSize,file_path, thread_num=3):
        self.header = header
        self.url = url
    ...
    def _update_progress_bar(self):
        ...
        self._show_message('已下载 {:.2f}%'.format(downloaded_percentage))
    ...
    def _show_message(self, message):
        ...
        ui_down_info_edit.verticalScrollBar().setValue(
...
class MyWidget(QWidget):
    ...
    def prepare_download(self):
        ...
        self._show_message("文件大小:{} B".format(self.allSize))
        ...
        if self.allSize > 1024 * 1024:
            message = "文件名：{}，文件大小：{:.2f} MB".format(file_path, self.allSize / (1024 * 1024))
        else:
            message = "文件名：{}，文件大小：{:.2f} kB".format(file_path, self.allSize / 1024 )

        choice = QMessageBox.question(self, "下载信息确认", message , QMessageBox.Yes | QMessageBox.No)
        ...
        if choice == QMessageBox.No:
            ...
            while not self.check_download_finished():
                time.sleep(0.1)
                #self._show_message("check_download_finished")
